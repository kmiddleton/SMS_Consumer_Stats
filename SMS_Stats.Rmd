...
Ski_Revenue <- c(1551, 1635, 1801, 1827, 1956, 1989, 2178, 2257, 2476, 2438)
D <- tibble(Year, Idaho_Divorces, Ski_Revenue)

cor_lab <- paste("Correlation =", round(cor(Idaho_Divorces, Ski_Revenue), 2))

D %>% gather(key = "key", value = "value", -Year) %>% 
  mutate(label = if_else(key == "Idaho_Divorces",
  ...
  theme(legend.position = "none",
        ...
        text = element_text(size = 20),
        axis.text = element_text(size = 18)) +
  geom_text(data = data.frame(Year = 2007,
...
Of that 9% who develop perennial AR, the comparison of >14 drinks per week relative to <1 drink per week, leads to a 78% increase in probability.
...
Smoking 15 or more cigarettes a day reduces the likelihood of AR.
